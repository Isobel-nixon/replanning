...
void CBS::savePaths(const string &fileName) const
{
    ...
    for (int i = 0; i < num_of_agents; i++)
    {
		...
		int prev_location = (*paths[i])[0].location;
		output << "[" << search_engines[0]->instance.getRowCoordinate(prev_location)
                   << "," << search_engines[0]->instance.getColCoordinate(prev_location) << "]";
        for (const auto & t : *paths[i]){
			if(t.location != prev_location){
		...
		heuristic_helper(instance.getDefaultNumberOfAgents(), paths, search_engines, initial_constraints, mdd_helper)
{
	...
	for (int i = 0; i < num_of_agents; i++)
	{
		...
		// Look at void ConstraintTable::insert2CT(size_t loc, int t_min, int t_max) to insert delays?
		// list<int> Instance::getNeighbors(int curr) const to get neighbour locations to insert delays
		if(search_engines[i] -> delay > 0){ 
			list<int> neighbours = search_engines[i]->getNeighbors(search_engines[i]->start_location);
			for(int location: neighbours){
				initial_constraints[i].insert2CT(location, 1, 1 + search_engines[i] -> delay);
			}
		}
